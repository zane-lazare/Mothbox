...
def update_webui_settings():
    ...
    try:
        ...
        try:
            ...
            stream_height = int(
                new_settings.get("stream_height", existing.get("stream_height", 768))
            )
        ...
        try:
            noise_reduction_mode = new_settings.get(
                "noise_reduction_mode", existing.get("noise_reduction_mode", 0)
            )
        ...
        try:
            colour_gains_red = float(
                new_settings.get("colour_gains_red", existing.get("colour_gains_red", 2.259))
            )
        ...
        try:
            ae_metering_mode = new_settings.get(
        ...
        try:
            exposure_time = int(
                new_settings.get("exposure_time", existing.get("exposure_time", 500))
            )
            analogue_gain = float(
        ...
        use_custom_tuning = new_settings.get(
            "use_custom_tuning", existing.get("use_custom_tuning", False)
        )
        if isinstance(use_custom_tuning, str):
            use_custom_tuning = use_custom_tuning.lower() == "true"

        lens_shading_enable = new_settings.get(
            "lens_shading_enable", existing.get("lens_shading_enable", True)
        )
        if isinstance(lens_shading_enable, str):
            lens_shading_enable = lens_shading_enable.lower() == "true"

        defect_correction_enable = new_settings.get(
            "defect_correction_enable", existing.get("defect_correction_enable", True)
        )
        ...
        focus_peaking_enabled = new_settings.get(
            "focus_peaking_enabled", existing.get("focus_peaking_enabled", False)
        )
        if isinstance(focus_peaking_enabled, str):
            focus_peaking_enabled = focus_peaking_enabled.lower() == "true"

        try:
            focus_peaking_intensity = int(
                new_settings.get(
                    "focus_peaking_intensity", existing.get("focus_peaking_intensity", 100)
                )
            )
        except (ValueError, TypeError) as e:
