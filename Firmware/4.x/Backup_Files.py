...
from mothbox_paths import DATA_DIR, MOTHBOX_HOME
...
internal_storage_minimum = (
    8  # This is Gigabytes, below 6 on a raspberry pi 5 can make weird OS problems
)
...
def rsync_photos_to_backup(source_dir, dest_dir):
    ...
    try:
        ...
        raise RuntimeError("Oh no! Mothbox couldn't backup your files!") from err
  ...
  """
    ...
    for disk_name, capacity in sorted_disks:
        ...
        if external_available > total_size_bytes:
            ...
            copy_folders_with_files(
                photos_folder, external_backup_folder
            )  # don't copy blank folders
            ...
            if differences:
                ...
                print(
                    f"Photos successfully copied to internal backup folder: {backedup_photos_folder}"
                )
