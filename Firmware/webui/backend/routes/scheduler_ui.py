...
@scheduler_ui_bp.route("/routines/preview", methods=["POST"])
@limiter.limit("30 per minute")
@require_json
def preview_routines(json_data: dict) -> tuple[Response, int]:
    """
    Preview execution times for unsaved routines.

    POST /api/scheduler/ui/routines/preview
    ...
    Request Body:
        {
            ...
            "days": 7,                // optional: preview days (default: 7, max: 90)
            "latitude": float,        // optional: for solar calculations
            "longitude": float,       // optional: for solar calculations
            "timezone": "string"      // optional: timezone name (default: UTC)
        }

    Returns:
        200 OK: Same structure as GET /schedules/{id}/preview
    ...
    try:
        ...
        if not routines_data:
            return jsonify({"error": "routines array cannot be empty"}), 400
        ...
        days_value = json_data.get("days", DEFAULT_PREVIEW_DAYS)
        try:
            days = min(max(int(days_value), 1), 90)
        ...
        valid, error = validate_timezone(timezone_name)
        if not valid:
            return jsonify({"error": f"Invalid timezone: {error}"}), 400

        # Build temporary schedule from routines
        temp_schedule = Schedule(
            schedule_id="preview-temp",
            name="Preview",
            routines=routines,
        )
        ...
        logger.error(f"Unexpected error in routines preview: {e}", exc_info=True)
        return jsonify(
            {
                "error": "Internal server error",
                "message": "Failed to generate preview",
