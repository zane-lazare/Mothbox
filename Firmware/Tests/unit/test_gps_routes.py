...
class TestGPSConfigUpdateEndpoint:
    ...
    def test_update_config_validates_timeout_ranges(self, client):
        ...
        assert response.status_code == 400
        assert 'gps_timeout_warm must be an integer between 30 and 180' in response.get_json()['error']

        # gps_timeout_cold: 60-300
        response = client.put('/api/gps/config', json={'gps_timeout_cold': 50})
        assert response.status_code == 400
        assert 'gps_timeout_cold must be an integer between 60 and 300' in response.get_json()['error']

        # gps_timeout_almanac: 300-1800
        response = client.put('/api/gps/config', json={'gps_timeout_almanac': 200})
        assert response.status_code == 400
        assert 'gps_timeout_almanac must be an integer between 300 and 1800' in response.get_json()['error']
    ...
    def test_update_config_updates_controls_file(self, client):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            response = client.put('/api/gps/config', json={
                'gps_enabled': True,
                'gps_timeout_hot': 15
            })
...
class TestGPSSyncEndpoint:
    ...
    def test_sync_uses_adaptive_timeout(self, client, temp_controls_file, tmp_path):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            mock_controls.return_value = {'gpstime': str(two_days_ago), 'lat': '37.7749', 'lon': '-122.4194', 'UTCoff': '0'}
    ...
    def test_sync_runs_gps_script_subprocess(self, client, temp_controls_file, tmp_path):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            mock_controls.return_value = {'gpstime': '0', 'lat': '37.7749', 'lon': '-122.4194', 'UTCoff': '0'}
    ...
    def test_sync_returns_success_with_fix(self, client, temp_controls_file, tmp_path):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            mock_controls.side_effect = [
                ...
                {'gpstime': '1234567890', 'lat': '37.7749', 'lon': '-122.4194', 'UTCoff': '-8'}  # After sync
            ]
...
class TestGPSCaching:
    ...
    def test_cache_returns_cached_data_within_ttl(self, client, temp_controls_file):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            mock_controls.return_value = {
                'lat': '37.7749', 'lon': '-122.4194', 'gpstime': '1234567890',
                'UTCoff': '0', 'gps_fix_mode': '3', 'gps_satellites_visible': '10',
                'gps_satellites_used': '8', 'gps_hdop': '1.2', 'gps_pdop': '2.1',
                'last_known_lat': 'n/a', 'last_known_lon': 'n/a', 'last_position_time': '0'
            }
    ...
    def test_cache_expires_after_ttl(self, client, temp_controls_file):
        ...
        with patch('routes.gps.get_hardware_config') as mock_hw, \
            ...
            mock_hw.return_value = {'gps_enabled': True}
            mock_controls.return_value = {
                'lat': '37.7749', 'lon': '-122.4194', 'gpstime': '0',
                'UTCoff': '0', 'gps_fix_mode': '0', 'gps_satellites_visible': '0',
                'gps_satellites_used': '0', 'gps_hdop': '99.99', 'gps_pdop': '99.99',
                'last_known_lat': 'n/a', 'last_known_lon': 'n/a', 'last_position_time': '0'
            }
