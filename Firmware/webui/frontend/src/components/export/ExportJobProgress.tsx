...
type JobPhase = 'initializing' | 'collecting' | 'exporting' | 'finalizing' | 'completed'
...
interface ExtendedExportJob extends ExportJob {
...
export interface ExportJobProgressProps {
  job?: ExtendedExportJob | null
  onCancel?: (jobId: string) => void
  onRetry?: () => void
}

export default function ExportJobProgress({ job, onCancel, onRetry }: ExportJobProgressProps) {
  if (!job) return null
  ...
  if (job.status === 'running') {
    const { current = 0, total = 0, percent = 0, phase = 'exporting' as JobPhase } = job.progress || {}
