...
class TestGetHardwareConfig:
    ...
    def test_returns_all_36_keys(self, tmp_path, monkeypatch):
        """Should return dict with all 36 hardware configuration keys"""
        import mothbox_paths
        ...
        # Should have exactly 36 keys (32 original + 4 I2C sensor keys from #230)
        # Sensor keys: light_sensor_*, temperature_sensor_*, i2c_bus (no motion sensor)
        assert len(config) == 36, f"Expected 36 keys, got {len(config)}"
        print("\n✓ All 36 hardware config keys present")
    ...
    def test_handles_missing_controls_file(self, tmp_path, monkeypatch):
        ...
        assert len(config) == 36  # 32 original + 4 I2C sensor keys from #230
        assert config['relay_enabled'] is True
        assert config['gps_device'] == '/dev/ttyAMA0'
        print("✓ Missing file returns all 36 defaults")
