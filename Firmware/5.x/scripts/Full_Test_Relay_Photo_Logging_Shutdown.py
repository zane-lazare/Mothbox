...
def takePhotowithCMD():
    ...
    cmd = (
        "libcamera-still --lens-position 7.4 -n  --width 9152 --height 6944 --awb cloudy --metering average --ev .5 -o "
        + filename
    )

    # cmd = "/usr/local/bin/libcamera-still --autofocus-mode manual -o "+filename
    subprocess.call(cmd, shell=True)  # nosec B602 - Test script, filename from controlled timestamp
