...
/**
 * Expected test fixtures for E2E tests
 * These fixtures are generated by Tests/fixtures/create_test_photos.py
 */
export const EXPECTED_FIXTURES = {
  'with-gps': [
...
/**
 * Verify test fixtures are deployed on the server
 *
 * Checks that expected test photo fixtures exist by fetching the gallery API
 * and verifying that expected filenames are present in the response.
 *
 * @param {import('@playwright/test').Page} page
 * @returns {Promise<{success: boolean, missing: string[], found: string[]}>}
 */
export async function verifyTestFixtures(page) {
  const allExpected = [
    ...EXPECTED_FIXTURES['with-gps'],
    ...EXPECTED_FIXTURES['without-gps'],
    ...EXPECTED_FIXTURES['hdr-series'],
    ...EXPECTED_FIXTURES['focus-bracket'],
  ]
  ...
  try {
    ...
    const response = await page.request.get('/api/gallery/photos')
    if (!response.ok()) {
      result.success = false
      result.missing = allExpected
      return result
    }

    const data = await response.json()
    const serverPhotos = new Set(data.photos?.map((p) => p.filename) || [])

    // Check which fixtures are present
    for (const fixture of allExpected) {
      if (serverPhotos.has(fixture)) {
        result.found.push(fixture)
      } else {
