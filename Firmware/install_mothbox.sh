...
if [ "$INSTALL_GPS_EXIF_SERVICE_FLAG" = "true" ] || [ "$INTERACTIVE_MODE" = "true" ]; then
    ...
    if [[ $INSTALL_GPS_EXIF_SERVICE =~ ^[Yy]$ ]]; then
        ...
        if [ ! -f "$SERVICE_SOURCE" ]; then
            ...
            sudo mkdir -p "/etc/systemd/system/${SERVICE_FILE}.d"

            # Detect current user and group for service
            ...
            # Create user override (applies to all installation types)
            echo "Creating systemd drop-in override for user configuration..."
            sudo tee "/etc/systemd/system/${SERVICE_FILE}.d/user-override.conf" > /dev/null <<EOF
# User/Group override
# Generated by install_mothbox.sh
# Overrides hardcoded User=pi and Group=pi in base service file
# This allows the service to work on non-Raspberry Pi OS systems
...
EOF
            ...
            echo -e "${GREEN}✓ Created user override: /etc/systemd/system/${SERVICE_FILE}.d/user-override.conf${NC}"
            echo "  User: $INSTALL_USER"
            echo "  Group: $INSTALL_GROUP"

            # For custom installations, create additional drop-in override for paths
            if [ "$INSTALL_TYPE" = "custom" ]; then
                echo "Creating systemd drop-in override for custom paths..."

                # Create override file with custom paths
                # Note: Use sudo tee to write with elevated permissions
                sudo tee "/etc/systemd/system/${SERVICE_FILE}.d/custom-paths.conf" > /dev/null <<EOF
# Custom installation path overrides
# Generated by install_mothbox.sh for INSTALL_TYPE=custom
# Custom installation path: $MOTHBOX_HOME

[Service]
# Override working directory
WorkingDirectory=$MOTHBOX_HOME

# Override environment variables
Environment="MOTHBOX_HOME=$MOTHBOX_HOME"
Environment="MOTHBOX_PHOTOS_DIR=$DATA_DIR/photos"
Environment="MOTHBOX_CONFIG_DIR=$CONFIG_DIR"

# Override filesystem protection paths
ReadWritePaths=$DATA_DIR/photos
ReadOnlyPaths=$MOTHBOX_HOME
EOF

                echo -e "${GREEN}✓ Created systemd drop-in override: /etc/systemd/system/${SERVICE_FILE}.d/custom-paths.conf${NC}"
            fi

            # Reload systemd to recognize new service and overrides
            ...
            echo "  User: $INSTALL_USER"
            echo "  Group: $INSTALL_GROUP"
            echo "  User override: /etc/systemd/system/${SERVICE_FILE}.d/user-override.conf"
            if [ "$INSTALL_TYPE" = "custom" ]; then
                echo "  Path override: /etc/systemd/system/${SERVICE_FILE}.d/custom-paths.conf"
                echo "  Installation: $MOTHBOX_HOME"
                echo "  Photos: $DATA_DIR/photos"
