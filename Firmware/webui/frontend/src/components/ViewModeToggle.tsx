 ...
 */
export default function ViewModeToggle({ currentView, onViewChange, isLoading = false }: ViewModeToggleProps) {
  // Normalize currentView to handle invalid values
  const validViews: ViewMode[] = ['grid', 'list', 'map']
  const normalizedView: ViewMode = validViews.includes(currentView as ViewMode) ? (currentView as ViewMode) : 'grid'

  // Warn in development if we received an unexpected non-null invalid value
  if (import.meta.env.DEV &&
      currentView != null &&
      !validViews.includes(currentView as ViewMode)) {
    console.warn(
