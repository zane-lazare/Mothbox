...
if [ "$INSTALL_GPS_EXIF_SERVICE_FLAG" = "true" ] || [ "$INTERACTIVE_MODE" = "true" ]; then
    ...
    if [[ $INSTALL_GPS_EXIF_SERVICE =~ ^[Yy]$ ]]; then
        ...
        if [ ! -f "$SERVICE_SOURCE" ]; then
            ...
            # For custom installations, create systemd drop-in override for paths
...
EOF
            ...
            echo ""
            if [ "$INSTALL_TYPE" = "custom" ]; then
                echo -e "${BLUE}Custom Path Configuration:${NC}"
                echo "  Installation: $MOTHBOX_HOME"
                echo "  Photos: $DATA_DIR/photos"
                echo "  Config: $CONFIG_DIR"
                echo "  Override file: /etc/systemd/system/${SERVICE_FILE}.d/custom-paths.conf"
                echo ""
            fi
