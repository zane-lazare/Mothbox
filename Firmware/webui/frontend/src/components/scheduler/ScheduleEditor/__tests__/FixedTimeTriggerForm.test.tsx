...
const defaultValue: FixedTimeTriggerValue = {
  time_of_day: '12:00',
  days_of_week: null,
}
...
describe('FixedTimeTriggerForm', () => {
  let mockOnChange: ReturnType<typeof vi.fn<(value: FixedTimeTriggerValue) => void>>

  beforeEach(() => {
    mockOnChange = vi.fn<(value: FixedTimeTriggerValue) => void>()
  })
  ...
  describe('Rendering', () => {
    ...
    it('renders with provided value', () => {
      const value: FixedTimeTriggerValue = {
        time_of_day: '18:30',
        days_of_week: [0, 1, 2],
      }
  ...
  describe('Time of Day Input (react-hook-form + Zod)', () => {
    ...
    it('shows Zod error for invalid time format', async () => {
      ...
      render(
        <FixedTimeTriggerForm value={defaultValue} onChange={mockOnChange} />,
      )
    ...
    it('does not propagate invalid values to parent', async () => {
      ...
      // Then assert no invalid call was made
      expect(mockOnChange).not.toHaveBeenCalledWith(
        expect.objectContaining({ time_of_day: 'bad' }),
      )
    })
    ...
    it('shows error and does not propagate when input is cleared', async () => {
      ...
      expect(mockOnChange).not.toHaveBeenCalledWith(
        expect.objectContaining({ time_of_day: '' }),
      )
    })
    ...
    it('shows parent-provided error message', () => {
      ...
      render(
        <FixedTimeTriggerForm onChange={mockOnChange} errors={errors} />,
      )
  ...
  describe('onChange Callback', () => {
    it('calls onChange with complete trigger object when time changes', async () => {
      const value: FixedTimeTriggerValue = {
        time_of_day: '12:00',
        days_of_week: [0, 1, 2],
      }
  ...
  describe('Prop sync (external value changes)', () => {
    it('updates input when value prop changes externally', () => {
      const value: FixedTimeTriggerValue = { ...defaultValue, time_of_day: '12:00' }

      const { rerender } = render(
        <FixedTimeTriggerForm value={value} onChange={mockOnChange} />,
      )

      rerender(
        <FixedTimeTriggerForm
          value={{ ...value, time_of_day: '18:00' }}
    ...
    it('does not reset form when value prop time_of_day is unchanged', async () => {
      const user = userEvent.setup()
      const value: FixedTimeTriggerValue = { ...defaultValue, time_of_day: '12:00' }
      ...
      // Parent re-renders with same time_of_day (e.g. days_of_week changed)
      rerender(
        <FixedTimeTriggerForm
          value={{ ...value, time_of_day: '12:00' }}
          onChange={mockOnChange}
        />,
      )
