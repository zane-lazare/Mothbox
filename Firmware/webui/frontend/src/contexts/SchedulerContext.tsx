 ...
 * }
 */
import React, { createContext, useReducer, useMemo, useCallback, useEffect, ReactNode, ErrorInfo } from 'react'
...
function loadFromStorage<T>(key: string, defaultValue: T): T {
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn(`Failed to load ${key} from localStorage:`, (error as Error).message)
    }
    return defaultValue
  }
}
...
function saveToStorage<T>(key: string, value: T): void {
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn(`Failed to save ${key} to localStorage:`, (error as Error).message)
    }
...
type SchedulerAction =
  | { type: 'SET_SCHEDULES'; payload: { schedules: Schedule[] } }
  | { type: 'SET_ACTIVE_SCHEDULE'; payload: { schedule: Schedule | null } }
  | { type: 'CLEAR_ACTIVE_SCHEDULE' }
  | { type: 'SET_EDITING_SCHEDULE'; payload: { schedule: Schedule | null } }
  | { type: 'CLEAR_EDITING_SCHEDULE' }
  | { type: 'UPDATE_EDITING_SCHEDULE'; payload: { updates: Partial<Schedule> } }
  | { type: 'SET_CREATING'; payload: { isCreating: boolean } }
  | { type: 'SET_UNSAVED_CHANGES'; payload: { hasChanges: boolean } }
  | { type: 'SET_PREVIEW'; payload: { events: PreviewEvent[] } }
  | { type: 'SET_PREVIEW_LOADING'; payload: { loading: boolean } }
  | { type: 'SET_PREVIEW_ERROR'; payload: { error: string | null } }
  | { type: 'CLEAR_PREVIEW' }
  | { type: 'SET_CONFLICTS'; payload: { conflicts: Conflict[] } }
  | { type: 'CLEAR_VALIDATION' }
  | { type: 'SET_MOON_PHASES'; payload: { phases: MoonPhases } }
  | { type: 'SET_VIEW_MODE'; payload: { mode: ViewMode } }
  | { type: 'SET_SELECTED_DATE'; payload: { date: string | null } }
  | { type: 'TOGGLE_EXPERT_MODE' }
  | { type: 'TOGGLE_DRAWER' }
  | { type: 'TOGGLE_SECTION'; payload: { section: string } }
  | { type: 'LOAD_STATE'; payload: { newState: Partial<State> } }
  | { type: 'RESET_STATE' }
  | { type: 'SET_ERROR'; payload: { error: Error; errorInfo: ErrorInfo | null } }
  | { type: 'CLEAR_ERROR' }
 ...
 */
function schedulerReducer(state: State, action: SchedulerAction): State {
  switch (action.type) {
     ...
     * to prevent prototype pollution attacks via malicious keys like __proto__ or constructor.
     *
     * Behavior:
    ...
    case ActionTypes.UPDATE_EDITING_SCHEDULE: {
      ...
      const sanitizedUpdates = Object.fromEntries(
        Object.entries(updates).filter(([key]) => SCHEDULE_FIELDS.includes(key as typeof SCHEDULE_FIELDS[number]))
      )
    ...
    case ActionTypes.LOAD_STATE: {
      ...
      const sanitizedState = Object.fromEntries(
        Object.entries(newState).filter(([key]) => LOADABLE_STATE_KEYS.includes(key as typeof LOADABLE_STATE_KEYS[number]))
      )
...
interface SchedulerContextValue {
  ...
  editActions: {
    ...
    updateEditingSchedule: (updates: Partial<Schedule>) => void
    setCreating: (isCreating: boolean) => void
    setUnsavedChanges: (hasChanges: boolean) => void
  }
  previewActions: {
    setPreview: (events: PreviewEvent[]) => void
    setPreviewLoading: (loading: boolean) => void
    setPreviewError: (error: string | null) => void
    clearPreview: () => void
    setMoonPhases: (phases: MoonPhases) => void
  }
  validationActions: {
  ...
  errorActions: {
    setError: (error: Error, errorInfo?: ErrorInfo | null) => void
    clearError: () => void
  }
  stateActions: {
...
export function SchedulerProvider({ children }: SchedulerProviderProps) {
  ...
  const setActiveSchedule = useCallback((schedule: Schedule | null) => {
    dispatch({
  ...
  const setEditingSchedule = useCallback((schedule: Schedule | null) => {
    dispatch({
  ...
  const updateEditingSchedule = useCallback((updates: Partial<Schedule>) => {
    dispatch({
  ...
  const setPreview = useCallback((events: PreviewEvent[]) => {
    dispatch({
  ...
  const setMoonPhases = useCallback((phases: MoonPhases) => {
    dispatch({
  ...
  const setError = useCallback((error: Error, errorInfo: ErrorInfo | null = null) => {
    dispatch({
  ...
  const contextValue = useMemo<SchedulerContextValue>(
    () => ({
      ...
      // Actions grouped by domain (all stable via useCallback)
      scheduleActions: {
        setSchedules,
        setActiveSchedule,
        clearActiveSchedule,
...
export class SchedulerErrorBoundary extends React.Component<SchedulerErrorBoundaryProps, SchedulerErrorBoundaryState> {
  static defaultProps = {
    fallback: null,
    onError: null,
  }
  ...
  static getDerivedStateFromError(): SchedulerErrorBoundaryState {
