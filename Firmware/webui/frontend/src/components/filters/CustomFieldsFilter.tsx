...
export function CustomFieldsFilter() {
  ...
  const [searchQueries, setSearchQueries] = useState<Record<string, string>>({}) // Track search per text field
  ...
  const handleSearchQueryChange = useCallback((fieldName: string, value: string) => {
    ...
    return (
      <div className="p-4">
        <div className="text-sm text-red-600 dark:text-red-400">
          Failed to load custom fields: {(error as Error)?.message || 'Unknown error'}
        </div>
      </div>
    )
  }
